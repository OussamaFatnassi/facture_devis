...
import { LoginUserUseCase, LoginUserRequest, LoginUserResponse, ValidateTokenRequest, ValidateTokenResponse } from '../application/use-cases/login-user';
import { AuthService } from '../application/services/auth-service';
import { PrismaUserRepository } from '../infrastructure/repositories/user-repository';
import { cookies } from 'next/headers';

export async function loginUser(formData: FormData): Promise<LoginUserResponse> {
  try {
    // Extract form data
...
export async function loginUserWithCredentials(email: string, password: string): Promise<LoginUserResponse> {
  try {
    // Create request object
    const request: LoginUserRequest = {
      email,
